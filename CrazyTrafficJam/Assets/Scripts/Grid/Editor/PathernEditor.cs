...
{
	[CustomEditor(typeof(Pathern))]
	public class PathernEditor : Editor
	{
		private Vector2Int size;
		private SerializedProperty nodesArray;
		private NodeType[,] grid;
		...
		private void Init()
		{
			...
			if (size.y < 1)
				size.y = 1;
			grid = new NodeType[size.x, size.y];

			for (int i = 0 ; i < nodesArray.arraySize ; ++i)
			{
				propertyPosition = nodesArray.GetArrayElementAtIndex(i).FindPropertyRelative("position").vector3IntValue;
				NodeType nodeType = (NodeType)nodesArray.GetArrayElementAtIndex(i).FindPropertyRelative("type").objectReferenceValue;
		...
		{
			NodeType[,] n = new NodeType[size.x, size.y];
					...
					switch (ev.type)
					{
						...
						case EventType.DragPerform:
							...
							if (DragAndDrop.objectReferences.Length != 1)
								break;
							NodeType dragType = DragAndDrop.objectReferences[0] as NodeType;
							if (dragType == null)
								break;
		...
		private void Save()
		{
			...
			for (int z = 0 ; z < size.y ; ++z)
			{
				for (int x = 0 ; x < size.x ; ++x)
				{
					if (grid[x, z] == null || grid[x, z].Type == ENodeType.None)
						continue;
